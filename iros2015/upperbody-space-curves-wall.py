...
inf = float('inf')
for pathid in range(0,len(paths)):
        print "HOMOTOPY class",pathid
        homotopyValues = np.array(outputValues[pathid])[:,0]
        validMinima = np.sum(homotopyValues < inf)

        pp = float(validMinima)/float(len(homotopyValues))

        bestMinima = np.argmin(homotopyValues)
        bestMinima = int(np.array(outputValues[pathid])[bestMinima,1])

        bestMinimaValue = np.min(homotopyValues)

        tharray = np.array(outputValues[pathid])[:,2]
        th = np.sum(tharray)
        print "Time for solving homotopy class:",outputTimes[pathid]

        print validMinima,"of",XspaceMinima,"are valid (",pp*100,"%)"
        print "best minima:",bestMinima,"with value",bestMinimaValue

        #upperBodyPathToFile(bestMinima, Aleftrightconv[bestMinima], x_WS, pathPlanes)
